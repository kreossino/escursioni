...
class escursioni_admin implements e_admin_addon_interface
{
	...
	//	$data = e107::getDb()->retrieve("escursioni","*", "escursioni_table='".$event."' AND escursioni_pid IN(".$ids.")",true);
	...
	*/
	...
	public function config(e_admin_ui $ui)
	{
		...
		//Note: 'urls' will be returned as $_POST['x_escursioni_url']. ie. x_{PLUGIN_FOLDER}_{YOURFIELDKEY}
	...
	public function process(e_admin_ui $ui, $id=null)
	{
		...
		switch($action)
		{
			...
			case 'edit':
					...
					{
						// eg. Save the data in the 'escursioni' plugin table.
					}
...
class escursioni_admin_form extends e_form
{
	 ...
	 * @return null|string
	 */
	function x_escursioni_custom($curval, $mode, $att=null) // 'x_' + plugin-folder + custom-field name.
	{
