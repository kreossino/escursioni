	...
	class escursioni_setup
	{
		...
		function install_post($var)
		{
			...
			$e107escursioni = array(
				'escursioni_id'				=> 0,
				'escursioni_icon'			=>'{e_PLUGIN}escursioni/images/escursioni_32.png',
				'escursioni_type'			=>'type_1',
				'escursioni_name'			=>'My Name',
				'escursioni_folder'			=>'Folder Value',
				'escursioni_version'			=>'1',
				'escursioni_author'			=>'1',
				'escursioni_authorURL'		=>'https://e107.org',
				'escursioni_date'			=>'1352871240',
				'escursioni_compatibility'	=>'2',
				'escursioni_url'				=>'https://e107.org',
				'escursioni_class'           => 0
			);

			/*
			 *   `escursioni_id` int(10) NOT NULL AUTO_INCREMENT,
				  `escursioni_icon` varchar(255) NOT NULL,
				  `escursioni_type` varchar(10) NOT NULL,
				  `escursioni_name` varchar(50) NOT NULL,
				  `escursioni_folder` varchar(50) NOT NULL,
				  `escursioni_version` varchar(5) NOT NULL,
				  `escursioni_author` varchar(50) NOT NULL,
				  `escursioni_authorURL` varchar(255) NOT NULL,
				  `escursioni_date` int(10) NOT NULL,
				  `escursioni_compatibility` varchar(5) NOT NULL,
				  `escursioni_url` varchar(255) NOT NULL,
				  `escursioni_class` int(10) NOT NULL,
			 */
		...
		function upgrade_required()
		{
			...
			// In this example, it checks if the table "escursioni_table" exists
//			if(!e107::getDb()->isTable('escursioni_table'))
//			{
...
//			}
			...
			// from the "escursioni_sql.php" file
			// In this case: Exists field "escursioni_id" in table "escursioni_table"
//			if(!e107::getDb()->field('escursioni_table','escursioni_id'))
//			{
//				return true;	 // true to trigger an upgrade alert, and false to not.
//			}
			...
			//
			// In this example, the field "escursioni_unused_field" from table "escursioni_table"
			// isn't used anymore and will be deleted (dropped) if it still exists
//			if(e107::getDb()->field('escursioni_table', 'escursioni_unused_field'))
//			{
...
//				e107::getDb()->gen("ALTER TABLE `#escursioni_table` DROP `escursioni_unused_field` ");
//			}
...
//			if(e107::getDb()->index('escursioni_table','escursioni_unused_index'))
//			{
				// this statement directly deletes the index, an additional
				// db update isn't needed anymore, if this is the only change on the db/table.
//				e107::getDb()->gen("ALTER TABLE `#escursioni_table` DROP INDEX `escursioni_unused_index` ");
//			}
			...
			// In this case, the index should be deleted if consists only of 1 field ("escursioni_fieldname"),
//			if(e107::getDb()->index('escursioni_table','escursioni_unused_index', array('escursioni_fieldname')))
//			{
				// this statement directly deletes the index, an additional
				// db update isn't needed anymore, if this is the only change on the db/table.
//				e107::getDb()->gen("ALTER TABLE `#escursioni_table` DROP INDEX `escursioni_unused_index` ");
//			}
			...
			// In this case, the index should be deleted if consists only of 1 field ("escursioni_fieldname"),
//			if ($index_def = e107::getDb()->index('escursioni_table','escursioni_unused_index', array('escursioni_fieldname')))
//			{
